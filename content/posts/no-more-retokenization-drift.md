+++
date = '2025-11-18T01:13:13+08:00'
draft = false
title = 'No More Retokenization Drift'
tags = ['retokenization drift', 'agent rl', 'vllm', 'openai-compatible api', 'agent lightning']
+++


# No More Retokenization Drift: Returning Token IDs via OpenAI Compatible APIs Matters in Agent RL
...
> Symptoms you’ll see in RL: unstable learning curves, and hard‑to‑debug divergence between the data you think you optimized on vs. what the model actually sampled.
![image](https://hackmd.io/_uploads/rJUpftz0xg.png)
...
* **Non-uniqueness of tokenization**. In practice this shows up constantly: a word might be produced during generation as two tokens (e.g., `H` + `AVING`), but when you re‑tokenize the text later during training you get a different split (e.g., `HAV` + `ING`). The text looks identical, but the *IDs differ*, making your learner optimizes against the wrong sequence.
![image](https://hackmd.io/_uploads/ByLAfYzCgx.png)
...
![image](https://hackmd.io/_uploads/HJA87tGRxx.png)
...
![image](https://hackmd.io/_uploads/r1tDQYGCxe.png)
